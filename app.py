...
st.markdown("""
    ...
    /* Khối nội dung Card trắng đổ bóng nhẹ */
    ...
    /* THIẾT KẾ BẢNG: CHỈ KẺ NGANG SIÊU MẢNH, KHÔNG KẺ DỌC */
    table { 
        width: 100% !important; 
    ...
    td { 
        ...
        border-bottom: 0.5px solid #e2e8f0 !important; /* Lằn kẻ ngang siêu mảnh */
        font-size: 13px !important;
    }
    tr:hover { background-color: #f1f5f9; }

    /* Ẩn các thành phần thừa khi in PDF */
    @media print {
...
if GSHEET_URL and GSHEET_URL != "CHÈN_LINK_GOOGLE_SHEET_CỦA_BẠN_VÀO_ĐÂY":
    ...
    if df is not None:
        ...
        try:
            ...
            df_summary = step1.groupby(order_col).agg({
                mother_col: 'count', cgl_len: 'sum', ccl_len: 'sum', cgl_width: 'mean', cgl_thick: 'mean', ccl_thick: 'mean'
            }).reset_index()
            ...
            st.table(sum_disp.set_index('No.').style.format({"Diff (m)": "{:.2f}", "Thick Var": "{:.3f}", "Diff Area (m²)": "{:.2f}"}))

            # --- PHẦN 2: BẢNG CHI TIẾT BABY COILS ---
            st.divider()
            st.subheader("🔍 2. 產出鋼捲明細 (Baby Coil Details)")
            selected_order = st.selectbox("Select Order Number to view details:", options=df[order_col].unique())
            if selected_order:
                df_det = df[df[order_col] == selected_order].copy()
                df_det['Var'] = df_det[ccl_thick] - df_det[cgl_thick]
                df_det_final = df_det[[mother_col, baby_col, cgl_thick, ccl_thick, 'Var', ccl_len]]
                df_det_final.columns = ['Mother Coil', 'Baby Coil', 'CGL Thick', 'CCL Thick', 'Var (mm)', 'CCL Len (m)']
                st.table(df_det_final.style.format({"CGL Thick": "{:.3f}", "CCL Thick": "{:.3f}", "Var (mm)": "{:.3f}", "CCL Len (m)": "{:.0f}"}))
            ...
            st.success("**💡 結論:** 分析厚度變化與長度增減的物理關聯。")
            ...
            * 📉 **長度短缺 (Length Shortfall):** 相當於 **{shortage_area:,.2f} m²** 的不明面積差異，需進一步調查原因。
            """)
            ...
            with c1:
                ...
                with pd.ExcelWriter(excel_bio, engine='xlsxwriter') as writer:
                    ...
                    if selected_order:
                        df_det_final.to_excel(writer, sheet_name='Order_Detail', index=False)
    ...
    st.info("👋 Vui lòng chèn Link Google Sheet vào biến `GSHEET_URL` ở dòng số 15 để App tự động làm việc.")
