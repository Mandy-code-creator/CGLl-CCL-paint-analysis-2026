...
# ==========================================================
# 1. THEME SELECTION & ANIMATED BACKGROUND
# ==========================================================
...
# CSS hiệu ứng màu nền chạy liên tục & Thủy tinh mờ
st.markdown(f"""
<style>
    .stApp {{
    ...
    /* Card style mờ ảo xuyên thấu nền */
    ...
    h1, h2, h3 {{ color: {text_color} !important; font-family: 'Segoe UI', sans-serif; font-weight: 700 !important; }}
...
# ==========================================================
# 2. CORE DATA LOGIC (KEEPING YOUR ORIGINAL)
# ==========================================================
    ...
    if df is not None:
        ...
        try:
           ...
           # --- UI: PRODUCTION COIL DETAILS ---
            ...
            st.plotly_chart(px.bar(disp, x='Order ID', y='Diff Area (m²)', color='Diff (m)', color_continuous_scale='Blues_r', template=plotly_template, title="Extra Area per Order"), use_container_width=True)
            st.info("**分析結論:** 監控各訂單的塗層面積偏差。")
            
            st.plotly_chart(px.bar(disp.sort_values(by='Cut Scrap (m)', ascending=False), x='Order ID', y='Cut Scrap (m)', color='Cut Scrap (m)', color_continuous_scale='Reds', template=plotly_template, title="Total Cut Scrap per Order"), use_container_width=True)
